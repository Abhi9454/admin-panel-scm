...
const AddStaff = () => {
  ...
  const handleSubmitMain = async (e) => {
    ...
    try {
      const payload = {
        ...
        user_type: formData.staff_type === 'Teacher' ? 'teacher' : 'staff',
      }
      ...
      if (formData.photo) {
        ...
        response = await staffManagementApi.create(multipartPayload)
      } else {
        response = await staffManagementApi.create(payload)
      }
      ...
      if (response && response.emp_code) {
        ...
        setAlertInfo({
          type: 'success',
          message: `🎉 Staff/Teacher added successfully! Employee Code: ${response.emp_code}. \nLogin Username: ${response.emp_code} \nDefault Password: ${formData.phone} \nYou can now fill in the additional information below.`,
        })
      } else {
        setAlertInfo({
          type: 'success',
          message: '🎉 Staff/Teacher added successfully!',
        })
      }
    } catch (error) {
      console.error('Error adding staff/teacher:', error)
      const errData = error?.response?.data
      let detailMsg = 'Failed to add staff/teacher.'
      if (errData) {
      ...
      setAlertInfo({
        type: 'danger',
        message: `❌ Error adding staff/teacher:\n${detailMsg}`,
      })
      alert(`Error adding staff/teacher:\n${detailMsg}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleUpdateAdditional = async (e) => {
    ...
    if (!createdEmpCode) {
      setAlertInfo({
        type: 'warning',
        message: '⚠️ Please add the main details above first before saving additional information.',
      })
      alert('Please add the main details above first before saving additional information.')
      return
    }
    ...
    try {
      ...
      await staffManagementApi.updateProfile(createdEmpCode, payload)
      setAlertInfo({
        type: 'success',
        message: '🎉 Additional details saved successfully for employee ' + createdEmpCode + '!',
      })
      ...
      {alertInfo && (
        <CCol xs={12}>
          <CAlert color={alertInfo.type} dismissible onClose={() => setAlertInfo(null)} className="shadow-sm">
            <div style={{ whiteSpace: 'pre-line' }}>{alertInfo.message}</div>
          </CAlert>
        </CCol>
      )}

      {/* Card 1: Add Staff Details */}
      <CCol xs={12}>
        <CCard className="shadow-sm mb-4">
          <CCardHeader className="py-2 px-3">
            <CRow className="align-items-center">
              <CCol md={6}>
                <h5 className="mb-0 fw-bold text-primary">Add Staff / Teacher</h5>
                <small className="text-muted">Enter basic details to register a new employee</small>
              </CCol>
              <CCol md={6} className="text-md-end text-start mt-2 mt-md-0">
                ...
                <CButton size="sm" color="outline-primary" onClick={() => navigate('/staff/directory')}>
                  ← Back to List
                </CButton>
              </CCol>
            </CRow>
            ...
            {loading ? (
              ...
              <CForm className="row g-3" onSubmit={handleSubmitMain}>
                ...
                <CCol md={12} className="mb-3">
                  <CFormLabel htmlFor="photo-upload-input" className="fw-semibold">Employee Photo</CFormLabel>
                  <CFormInput
                    type="file"
              ...
              <CCol md={6} className="text-md-end text-start mt-2 mt-md-0">
                {createdEmpCode ? (
                  ...
                  <CBadge color="warning" className="px-2 py-1 text-dark">
                    🔒 Locked: Add employee details above first
                  </CBadge>
                )}
              </CCol>
            </CRow>
            ...
            {!createdEmpCode ? (
              <div>
                ...
                <p className="mb-0 small text-secondary">
                  Please fill out the Name, Email, Phone, and Gender above and click <strong>Add Staff / Teacher</strong> to generate an Employee Code first.
                </p>
              </div>
            ) : (
              <>
