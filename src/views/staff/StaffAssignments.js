...
const StaffAssignments = () => {
  ...
  const loadDropdowns = async () => {
    try {
      const [teacherData, classData, sectionData, subjectData] = await Promise.all([
        staffManagementApi.getAll({ page_size: 200, user_type: 'teacher' }),
  ...
  const fetchAssignments = async () => {
    try {
      ...
      const response = await staffManagementApi.getAssignments(params)
        ...
        <CCard className="shadow-sm">
          <CCardHeader className="py-2 px-3">
            <CRow className="align-items-center g-2">
              ...
              <CCol lg={5} md={2} sm={6} className="d-flex gap-2 justify-content-end">
                <CButton
                  size="sm"
                ...
                <CTable hover small className="mb-0">
                  ...
                  <CTableBody>
                    {assignments.map((a) => {
                      const teacherName =
                        a.teacher_name ||
                        teachers.find((t) => t.emp_code === a.emp_code)?.emp_name ||
                        'N/A'
                      const className =
                        a.class_title ||
                        classes.find((c) => c.id === a.class_id)?.title ||
                        a.class_id
                      const sectionName =
                        a.section_title ||
                        sections.find((s) => s.id === a.section_id)?.title ||
                        a.section_id
                      const subjectName =
                        a.subject_title ||
                        (a.subject_id
