...
const StaffAttendance = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await staffManagementApi.getAttendance(params)
  ...
  const fetchSummary = async () => {
    if (!filterEmpCode || !filterFromDate || !filterToDate) {
      setAlert({
        type: 'warning',
        message: 'Please enter Emp Code, From Date, and To Date to view summary.',
      })
      return
    }
    ...
    setSummary(null)
    try {
      const data = await staffManagementApi.getAttendanceSummary({
  ...
  const handleMarkSubmit = async (e) => {
    ...
    try {
      ...
      await staffManagementApi.markAttendance({
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const formatTime = (t) => (t ? (t.split(':').length === 2 ? `${t}:00` : t) : undefined)
      await staffManagementApi.updateAttendance(editRecord.id, {
  ...
  return (
    <CRow className="g-2">
          ...
          <CCard className="shadow-sm border-start border-4 border-primary">
            <CCardBody className="py-2 px-3">
              <CRow className="align-items-center">
                ...
                <CCol xs="auto">
                  <CButton size="sm" color="outline-secondary" onClick={() => setSummary(null)}>
                    Close
                  </CButton>
                </CCol>
              </CRow>
              <CRow className="g-2 mt-1">
                {[
                  {
                    label: 'Total Days',
                    value: summary.summary?.total_working_days,
                    color: 'secondary',
                  },
        ...
        <CCard className="shadow-sm">
          <CCardHeader className="py-2 px-3">
            <CRow className="align-items-center g-2">
                ...
                >
                  ...
                  {teachers.map((t) => (
                    <option key={t.emp_code} value={t.emp_code}>
                      [{t.emp_code}] {t.emp_name} ({t.user_type === 'staff' ? 'Staff' : 'Teacher'})
                    </option>
                  ))}
                </CFormSelect>
              </CCol>
              ...
              {/* Action Buttons */}
              <CCol lg={2} md={12} className="d-flex gap-2 justify-content-end flex-wrap">
                <CButton
                  size="sm"
                  ...
                  <CTableBody>
                    {records.map((r) => (
                      <CTableRow key={r.id} className="align-middle">
                        ...
                        <CTableDataCell className="py-2 px-3">{r.check_in_time || '—'}</CTableDataCell>
                        <CTableDataCell className="py-2 px-3">{r.check_out_time || '—'}</CTableDataCell>
                        <CTableDataCell className="py-2 px-3">
      ...
      {/* Mark Attendance Modal */}
      <CModal visible={markModalVisible} onClose={() => setMarkModalVisible(false)} size="lg">
        <CModalHeader>
          <CModalTitle>Mark Staff Attendance</CModalTitle>
        </CModalHeader>
