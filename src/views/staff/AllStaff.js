...
const AllStaff = () => {
  ...
  useEffect(() => {
    fetchTeachers()
  }, [currentPage, selectedStatus, selectedUserType, searchTerm])
  ...
  const fetchTeachers = async () => {
    try {
      ...
      const response = await staffManagementApi.getAll(params)
  ...
  const handleView = (empCode) => {
    navigate('/staff/edit', { state: { empCode } })
  }

  const handleAttendance = (empCode) => {
    navigate('/staff/attendance', { state: { empCode } })
  }

  const handleAssignments = (empCode) => {
    navigate('/staff/assignments', { state: { empCode } })
  }
        ...
        <CCard className="shadow-sm">
          ...
          <CCardHeader className="py-2 px-3">
            <CRow className="align-items-center g-2">
              ...
              {/* Actions */}
              <CCol lg={3} md={4} sm={12} className="d-flex gap-2 justify-content-end">
          ...
          {loading ? (
            ...
            <div className="text-center py-5">
              <div className="text-muted">
                <p className="mb-2">No staff or teachers found matching your criteria</p>
                  ...
                  <CTableBody>
                    {teachers.map((teacher) => (
                      <CTableRow key={teacher.emp_code} className="align-middle">
                        ...
                        <CTableDataCell className="py-2 px-3 border-end">
                          <CBadge color={teacher.user_type === 'staff' ? 'secondary' : 'primary'}>
                            {teacher.user_type === 'staff' ? 'Staff' : 'Teacher'}
                          </CBadge>
                        </CTableDataCell>
                        <CTableDataCell className="py-2 px-3 border-end">
                          <CBadge color={STATUS_COLORS[teacher.status] || 'dark'}>
                            {teacher.status || 'N/A'}
                          </CBadge>
                        </CTableDataCell>
